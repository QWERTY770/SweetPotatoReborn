...
import io.github.qwerty770.mcmod.spmreborn.util.inventory.MagicCubeInputSlot;
...
public class MagicCubeScreenHandler extends AbstractContainerMenu {
    ...
    public MagicCubeScreenHandler(int syncId, Inventory playerInventory, Level world, BlockPos pos, Container inventory, IntMagicCubeProperties properties) {
        super(SweetPotatoMenuTypes.MAGIC_CUBE_SCREEN_HANDLER_TYPE.get(), syncId);
        ...
        mainFuelSlot = this.addSlot(new FuelSlot(SweetPotatoItems.PEEL.get(), inventory, 6, 12, 32));
        viceFuelSlot = this.addSlot(new FuelSlot(SweetPotatoItems.POTATO_POWDER.get(), inventory, 7, 139, 30));
    ...
    @Override
    public @NotNull ItemStack quickMoveStack(Player playerEntity, int index) {
            ...
            if (index > 7) {
                if (this.viceFuelSlot.mayPlace(itemStack)) {
                    ...
                    if (!this.moveItemStackTo(itemStack2, 6, 7, false))
                        return ItemStack.EMPTY;
                } else if (SPRMain.RAW_SWEET_POTATOES.contains(itemStack.getItem()) && itemStack.getCount() == 1) {
